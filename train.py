...
while True:
    ...
    #                     param *= mask_dict[name]
        ...
        for block in model.transformer.h:
            block.mlp.pruneFlag = False # Set to False to disable pruning ???
                                        # Note we don't set the pruned flag to False here.
        # for name, param in model.named_parameters():
        #     if "weight" in name:
        #         with torch.no_grad(): # freezes the grad for the entire matrix??? or just the mask?
        #             param *= mask_dict[name]
    ...
    for name, param in raw_model.named_parameters():
        if "weight" in name:
            nonzero_values += (param != 0).sum().item()

    percent_nonzero_values = (nonzero_values / total_values) * 100
    print("Percentage of weight values that equal 0:", percent_nonzero_values)
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                checkpoint = {
                    ...
                    "percent_nonzero": percent_nonzero_values,
                }
...
#                   bias=bias, vocab_size=None, dropout=dropout) # start with model_args from command line
# if init_from == 'scratch':
#     print("in the scratch init_from")
#     # init a new model from scratch
#     print("Initializing a new model from scratch")
#     # determine the vocab size we'll use for from-scratch training
#     if meta_vocab_size is None:
...
# #     'config': config,
# # }
# # out_dir='pretrainedMediumBatchSize1'
# # print(f"saving checkpoint to {out_dir}")
# # torch.save(checkpoint, os.path.join(out_dir, 'ckpt.pt'))
# # print(f"done saving the pretrained")
...
# #     'best_val_loss': best_val_loss,
# #     'config': config,
# #     'timeSeconds': timecompleted
# # }
# # out_dir = "shakespeare"
# # print(f"saving checkpoint to {out_dir}")
# # torch.save(checkpoint, os.path.join(out_dir, 'ckpt.pt'))
# # print("final checkpoint saved with the time also recorded to train")
