...
while True:
    ...
    if (prune_type == "zero_indiv"):
        ...
        if (iter_num == prune_at):
            ...
            for block in model.transformer.h:
                ...
                block.mlp.prune_percentile = prune_percentile
    ...
    for name, param in raw_model.named_parameters():
        if "weight" in name:
            non_zero_values += (param != 0).sum().item()

    percent_non_zero_values = (non_zero_values / total_values) * 100
    print("Percentage of weight values that are nonzero:", percent_non_zero_values)
    # evaluate the loss on train/val sets and write checkpoints
    print(iter_num, eval_interval, iter_num % eval_interval)
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            time_so_far = time.time() - start_train
            if iter_num > 0:
                checkpoint = {
                    ...
                    'config': config,
                    'param_pct_nonzero': percent_non_zero_values,
